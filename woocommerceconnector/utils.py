...
def disable_woocommerce_sync_on_exception(config):
    frappe.db.rollback()
    frappe.db.set_value("WooCommerce Config", config, "enable_woocommerce", 0)
    frappe.db.commit()

def is_woocommerce_enabled(config=None):
    if config:
