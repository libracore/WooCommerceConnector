...
def check_hourly_sync():
    hosts = frappe.get_all("WooCommerce Config", fields=['name'])
    for host in hosts:
        woocommerce_settings = frappe.get_doc("WooCommerce Config", hosts['name'])
        if woocommerce_settings.hourly_sync == 1:
            sync_woocommerce(woocommerce_settings.name)

@frappe.whitelist()
def sync_woocommerce(config=None):
    "Enqueue longjob for syncing woocommerce"
    enqueue(method="woocommerceconnector.api.sync_woocommerce_resources", 
        queue='long', 
        timeout=1500,
        **{'config': config})
...
@frappe.whitelist()
def sync_woocommerce_resources(config=None):
    frappe.log_error(config)
    if config:
        woocommerce_settings = frappe.get_doc("WooCommerce Config", config)
    else:
    ...
    if woocommerce_settings.enable_woocommerce:
        ...
        try :
            ...
            sync_products(woocommerce_settings.price_list, woocommerce_settings.warehouse, 
                woocommerce_settings, True if woocommerce_settings.sync_items_from_woocommerce_to_erp == 1 else False)
            sync_customers(woocommerce_settings)
            sync_orders(woocommerce_settings)
            # close_synced_woocommerce_orders() # DO NOT GLOBALLY CLOSE
            update_item_stock_qty(woocommerce_settings)
            frappe.db.set_value("WooCommerce Config", config, "last_sync_datetime", frappe.utils.now())
            ...
            if e.args[0] and hasattr(e.args[0], "startswith") and e.args[0].startswith("402"):
                ...
                disable_woocommerce_sync_on_exception(config)
...
def validate_woocommerce_settings(woocommerce_settings):
    ...
    try:
        ...
        disable_woocommerce_sync_on_exception(woocommerce_settings.name)
